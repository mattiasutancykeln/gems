...
<a id="g18-f003"></a>
### Skills are static markdown documents loaded on-demand rather than pre-injected into the…
...
<a id="g18-f004"></a>
### When skills are registered the tool injects their names as an enum constraint on the…
...
<a id="g18-f006"></a>
### drain_notifications() is a pull-based push model: background work accumulates…
...
<a id="g18-f007"></a>
### process_result persists large tool outputs to disk and returns a head+tail preview plus a…
...
<a id="g18-f008"></a>
### ExecutorTool is added to the tool list only when a parent_agent is explicitly passed,…
...
<a id="g18-f009"></a>
### Child config deep-copies the parent's llm / timeout / context subgroups wholesale but…
...
<a id="g18-f010"></a>
### Child agents strip Executor from their tool list by name to prevent recursive spawning
...
<a id="g18-f011"></a>
### _classify_executor_outcome distinguishes done (real metric produced or eval intentionally…
...
<a id="g18-f012"></a>
### The IDEATE body switches at runtime between a strict skill-driven Stage A-E flow and a…
...
<a id="g18-f013"></a>
### coerce_str_list tries json.loads , then ast.literal_eval , then URL regex extraction in order, so LLM callers can pass raw strings, JSON arrays, or informal comma-lists interchangeably
...
<a id="g18-f014"></a>
### eval_cmd uses {cwd} and {node_id} template variables substituted at dispatch time so each executor gets its own worktree path and a unique result directory prefix
...
<a id="g18-f015"></a>
### Path guard blocks mlebench/data/.../private at both the file-path level (via…
...
<a id="g18-f017"></a>
### asyncio.wait_for with a configurable nested_executor_timeout bounds child execution
...
<a id="g18-f018"></a>
### Glob results drop hidden/VCS directories ( {".git", ".svn", ".hg", "node_modules",…
...
<a id="g18-f023"></a>
### ExecutorTool.description is a structured LLM-facing briefing guide explaining semantics,…
...
<a id="g18-f024"></a>
### GlobTool.description includes an in-band routing hint directing the LLM toward Executor…
...
<a id="g18-f027"></a>
### build_system_prompt builds from 9 named sections in a documented order
...
<a id="g18-f029"></a>
### Tool description embeds LLM-facing behavioral contracts inline: "MUST use the Read tool…
...
<a id="g18-f030"></a>
### Edit tool description encodes the read-before-edit contract, the uniqueness requirement…
...
<a id="g18-f031"></a>
### Execute returns the skill body prefixed with a rendered Markdown header ( # Skill: {name}…
...
<a id="g18-f032"></a>
### Tool description inlines when_to_apply hints from skill metadata so the LLM knows at…
...
<a id="g18-f033"></a>
### A two-column table maps every native Arbor tree/eval/merge operation to a deterministic…
...
<a id="g18-f034"></a>
### build_coordinator_system_prompt conditionally includes _grounded_ideation_section and…
...
<a id="g18-f035"></a>
### Structured extraction prompt using {webpage_content} and {goal} template variables, returning JSON with rational / evidence / summary fields
...
<a id="g18-f036"></a>
### Strips url: prefixes that LLMs emit when copying from tool descriptions, and trailing ,]…
...
<a id="g18-f037"></a>
### to_api_schema() serializes tools to the Anthropic API {name, description, input_schema}…
...
<a id="g18-f038"></a>
### Full executor lifecycle as a single _run_single_executor coroutine: validate -> create worktree -> snapshot protected paths -> run agent under asyncio.wait_for -> tamper check -> finalize/remove worktree -> LLM-parse report -> update tree -> propagate insights -> emit events -> save artifacts -> format summary
...
<a id="g18-f047"></a>
### HITL review gate ( _review_gate ): in review / collaborative mode, pauses before spending compute
...
<a id="g18-f049"></a>
### _fetch_page chains Jina then raw-requests fallback, returning a canned error string only when both fail
...
<a id="g18-f051"></a>
### Two-factory pattern ( build_web_search_tool , build_web_visit_tool ) with getattr(sc, key, default) duck-typing
...
<a id="g18-f052"></a>
### get_all_tools() factory reads all timeout/limit config from AgentConfig with inline numeric defaults
...
<a id="g18-f054"></a>
### Consistent path-guard check before any I/O ( check_path_allowed(path) ) returning a…
...
<a id="g18-f058"></a>
### Convergence signal in RunExecutorParallel iterates over all tasks but overwrites signal on each iteration without breaking
...
<a id="g18-f059"></a>
### except Exception as e swallows all non-timeout child errors into a generic string
...
<a id="g18-f060"></a>
### When disk persistence fails, process_result silently falls back to _truncate with no warning to caller or model
...
<a id="g18-f061"></a>
### No length cap on the evidence field ("output the full original context … as far as possible, it can be more than three paragraphs")
...
<a id="g18-f062"></a>
### _BLOCKED_PATH_PATTERNS is hardcoded to the mlebench layout and is not configurable via constructor or config
...
<a id="g18-f063"></a>
### _ask_user_section has two independent conditions for inclusion ( allow_agent_questions OR interaction_mode in ("direction", "collaborative") )
...
<a id="g18-f064"></a>
### Binary file detection is skipped when mimetypes.guess_type returns None (unknown extension: .go , .rs , .ts , etc
...
<a id="g18-f065"></a>
### The trailing-whitespace fuzzy match (tier 3) uses file_stripped.find(search_stripped) then slices file_content[idx: idx + len(search_stripped)] using the stripped length
...
<a id="g18-f066"></a>
### Recursion guard is name-based only ( t.name != "Executor" )
...
<a id="g18-f067"></a>
### The truncation notice ( [Showing first 100 of more matches] ) is appended as a plain string to the path list
...
<a id="g18-f068"></a>
### PDF reading depends solely on subprocess pdftotext (poppler-utils)
...
<a id="g18-f069"></a>
### JinaVisitTool.__init__ calls Tool.__init__ directly, bypassing WebVisitTool.__init__
...
<a id="g18-f070"></a>
### ResumeExecutor checks node.code_ref is None and returns an error, but a…
...
<a id="g18-f071"></a>
### Worktree creation failure resets node to pending (not needs_retry ) on the grounds that…
...
<a id="g18-f072"></a>
### The truncation policy tells the agent to treat a short snippet as sufficient evidence but provides no fallback if the snippet is genuinely empty (e.g., a visit returning a 0-byte body)
