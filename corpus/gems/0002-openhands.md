...
**`RouterLLM.__getattr__` silent fallback to first LLM** (`router/base.py:107-111`): attribute lookup failures fall through to the first LLM in the routing dict without warning. Misconfigured routing (e.g., wrong model key) fails silently.
