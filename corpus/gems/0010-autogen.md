...
<a id="g10-f003"></a>
### Active speakers list tracks which agents are currently processing. Responses (agent or team) remove the respondent fr…
...
<a id="g10-f004"></a>
### AgentTool and TeamTool wrap agents/teams as callable tools for other agents. This inversion
...
<a id="g10-f006"></a>
### pause() and resume() are separate from reset() . reset() clears accumulated history; pause/resume are lifecycle signa…
...
<a id="g10-f007"></a>
### Streaming and blocking modes share a single underlying run_stream() coroutine; run() collects all outputs. The interf…
...
<a id="g10-f008"></a>
### Outer loop (facts+plan ledger) is re-entered via _reenter_outer_loop , which resets all agents (L265-270), clears the…
...
<a id="g10-f009"></a>
### _init() is called lazily on first run_stream() (line 493) and also on reset() if not yet initialized. Defers agent re…
...
<a id="g10-f010"></a>
### GroupChatStart message is sent as a single RPC to the manager with all task messages and a boolean flag ( output_task…
...
<a id="g10-f011"></a>
### Handoff.target is a string (agent name), not a reference. Downstream orchestrator must resolve name to agent instance…
...
<a id="g10-f012"></a>
### Participant names are stored alongside topic types in a name-to-topic-type mapping dict, decoupling agent identity (n…
...
<a id="g10-f013"></a>
### FIFOLock enforces strict ordering of message processing by type using an event queue + flag, preventing message inter…
...
<a id="g10-f014"></a>
### Agents are declared stateful; only new messages since last call should be passed and agents maintain internal state a…
...
<a id="g10-f015"></a>
### Introduces output_task_messages parameter to avoid fragile count-based logic when filtering task messages from inner …
...
<a id="g10-f018"></a>
### Team ID is bound to the group chat object instance, not regenerated per run, making team participants always have sta…
...
<a id="g10-f019"></a>
### ChatAgent.on_messages() returns Response with inner_messages as optional, bifurcating public output ( chat_message ) …
...
<a id="g10-f020"></a>
### TerminationCondition combines via operator overload ( __and__ , __or__ ), not factory methods, embedding composition …
...
<a id="g10-f022"></a>
### Facts prompt solicits pre-survey of GIVEN/LOOKUP/DERIVE/EDUCATED facts. Separates known from unknown before planning,…
...
<a id="g10-f024"></a>
### Facts update prompt is called on stall; re-prompts model to edit educated guesses in light of failure, moving guesses…
...
<a id="g10-f025"></a>
### Plan update prompt asks model to identify root cause of stall, then devise a new plan. Explicit replay prevention.
...
<a id="g10-f027"></a>
### StructuredMessageFactory accepts either a Pydantic model or a JSON schema, generates a strongly-typed StructuredMessa…
...
<a id="g10-f029"></a>
### tool_call_summary_formatter is a Pydantic-incompatible callable that customizes tool result presentation, allowing co…
...
<a id="g10-f032"></a>
### FunctionalTermination detects async callables via asyncio.iscoroutinefunction() at call-time (line 216), not at const…
...
<a id="g10-f034"></a>
### Task input accepts str , single BaseChatMessage , or list of BaseChatMessage ; all normalized to a message list and v…
...
<a id="g10-f035"></a>
### Abstract select_speaker() returns a single speaker name ( str ) or list ( List[str] ); allows multi-speaker turns (pa…
...
<a id="g10-f037"></a>
### Adaptive LLM message formatting based on model capability ( multiple_system_messages ). Falls back to UserMessage wra…
...
<a id="g10-f038"></a>
### custom_message_types parameter allows injection of new message types at team creation time; custom types are register…
...
<a id="g10-f055"></a>
### Output message queue consumer loop has no timeout or max-iteration count. If a bug in the manager causes infinite mes…
...
<a id="g10-f056"></a>
### TokenUsageTermination has no per-child budget tracking; total token consumption is flat across all messages. No way t…
...
<a id="g10-f057"></a>
### _active_speakers is a simple list, not a set. If select_speaker returns the same agent twice, duplicates are possible…
...
<a id="g10-f058"></a>
### Mutual exclusion check is a flag ( _is_running ); no lock guards against concurrent run() calls in multi-threaded sce…
...
<a id="g10-f059"></a>
### _max_json_retries = 10 is hardcoded with no exposed configuration knob. If the model fails to produce valid ledger JS…
...
<a id="g10-f060"></a>
### select_speaker is awaited as a future but there is no timeout or circuit breaker. If select_speaker hangs (e.g., wait…
...
<a id="g10-f061"></a>
### Swarm explicitly does NOT support inner teams as participants, only ChatAgent . Limits composition depth; no nested-t…
...
<a id="g10-f062"></a>
### MagenticOne raises RuntimeError if it receives GroupChatTeamResponse instead of GroupChatAgentResponse . Does not sup…
...
<a id="g10-f063"></a>
### MessageFactory is a flat registry with no versioning or migration path. If two agents are compiled with different mes…
...
<a id="g10-f064"></a>
### parallel_tool_calls=False is a hardcoded requirement for any model client driving an agent or team tool. No automatic…
...
<a id="g10-f065"></a>
### If reset() itself fails, the team is left in _is_running=True , preventing further runs. Exception handling propagate…
...
<a id="g10-f066"></a>
### OrTerminationCondition raises RuntimeError , while AndTerminationCondition raises TerminatedException . Inconsistent …
...
<a id="g10-f067"></a>
### If multiple handoffs are detected, only the first is executed. Parallel tool calls can trigger unintended handoffs un…
...
<a id="g10-f069"></a>
### assert result is not None will crash if the inner team yields only events and no TaskResult . No explicit guard or ti…
...
<a id="g10-f071"></a>
### Output message queue is unbounded ( asyncio.Queue() with no maxsize ). High-volume message production (e.g., streamin…
...
<a id="g10-f072"></a>
### Agent name must be a valid Python identifier ( isidentifier() ). Enforced at construction but not reflected in type h…
