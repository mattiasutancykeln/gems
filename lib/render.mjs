...
function clusterLine(c) {
  return `- "${c.label}" — ${c.size} findings, gems ${c.gems.map((g) => `#${g}`).join(", ")}`;
}

export function renderFacets(facets) {
  const { topics, categories, codeReuse, licenses, repos, clusters, totals } = facets;
  const shownRepos = repos.slice(0, REPO_CAP);
  const shownClusters = clusters.slice(0, CLUSTER_CAP);
  const moreRepos = repos.length - shownRepos.length;
  const moreClusters = clusters.length - shownClusters.length;
  ...
  const parts = [
    ...
    `- ${joinCounts(topics)}`,
    "### Categories",
    `- ${joinCounts(categories)}`,
    "### Code reuse",
    `- ${joinCounts(codeReuse)}`,
    "### Licenses",
    `- ${joinCounts(licenses)}`,
    "### Top source repos",
    `- ${joinCounts(shownRepos)}`,
  ];
  if (moreRepos > 0) parts.push(`... and ${moreRepos} more`);
  parts.push(`### Cross-gem clusters (${clusters.length})`);
  if (!shownClusters.length) parts.push("(none)");
  else parts.push(...shownClusters.map(clusterLine));
  if (moreClusters > 0) parts.push(`... and ${moreClusters} more`);
