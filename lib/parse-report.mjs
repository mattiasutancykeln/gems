...
export function parseIssue(issue) {
  ...
  if (reportComment) {
    ...
    if (sumComment) {
      ...
      // raw/summarized gems legitimately have no extraction report yet - that
      // is a pipeline stage, not a defect, so don't warn for them.
      const stage = label(labels, "stage:");
      if (stage !== "raw" && stage !== "summarized") {
        warnings.push(`gem #${issue.number}: no parseable report or highlights comment`);
      }
    }
  }
