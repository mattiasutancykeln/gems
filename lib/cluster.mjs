...
// Exact Jaccard similarity of two shingle sets: |intersection| / |union|.
// Iterates the smaller set for the intersection check, since these sets are
// small (word unigrams/bigrams per finding) this stays cheap even across
// the full ~800k-pair all-pairs sweep at corpus-sync time.
...
export function clusterFindings(findings, { shingleK = 2, threshold = 0.15 } = {}) {
  const shingles = findings.map((f) =>
    contentShingles(cleanContentTokens(f.title + " " + f.text), shingleK));
  const uf = new UnionFind(findings.length);
  for (let i = 0; i < findings.length; i++) for (let j = i + 1; j < findings.length; j++) {
    // A finding with no surviving content tokens (e.g. its text was pure
    // citation/code-span/URL noise) has an empty shingle set. An empty set's
    // Jaccard similarity against anything is defined as 0 above, so this
    // guard is belt-and-suspenders, but keep it explicit: two empty-content
    // findings should never merge on the basis of sharing no actual content.
    if (shingles[i].size === 0 || shingles[j].size === 0) continue;
    if (jaccard(shingles[i], shingles[j]) >= threshold) uf.union(i, j);
  }
