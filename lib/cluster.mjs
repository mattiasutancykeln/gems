...
export function clusterFindings(findings, { shingleK = 2, numHashes = 64, threshold = 0.1 } = {}) {
  ...
  const sigs = shingles.map((sh) => minhashSig(sh, numHashes));
  ...
  for (let i = 0; i < findings.length; i++) for (let j = i + 1; j < findings.length; j++) {
    ...
    // citation/code-span/URL noise) has an empty shingle set. minhashSig
    // leaves every hash slot at its untouched sentinel (0xffffffff) for an
    // empty set, so two empty-content findings would otherwise compare as a
    // perfect match and get union-merged despite sharing no actual content.
    // Skip pairwise union whenever either side's shingle set is empty; each
    // such finding keeps its own singleton cluster.
